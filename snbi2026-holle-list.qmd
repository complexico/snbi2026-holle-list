...
After loading the relevant functions by executing codes in @fig-module-load, we need to write custom Python codes for the digitalisation processes. An example is shown in @fig-python-ocr for the processing of the regional Holle List vol. 5.1 for the Papuan languages in the Digul Area, Irian Jaya/West Papua of Indonesia.

![Custom Python codes for the digitalisation of the PDF into plain text](img/google-colab-digitalisation.png){#fig-python-ocr}

Codes in the upper code block/box in @fig-python-ocr deal with converting the PDF into image by providing the Google Drive path of the PDF. After that come the codes in the lower code block/box in @fig-python-ocr. They cover (i) setting-up the parameters for the output format of the plain text, (ii) creating the main processing iteration to OCR each image page, and then (iii) saving the output into a plain text file (see the lower code block/box in @fig-python-ocr).

Note that writing the codes does not mean that the codes are automatically executed/instructed to produce the output. To run the codes inside the code block, hover the mouse in the top-left area of the code block until a white rightward arrow (with black background) appears (see the point of the blue arrow in @fig-executing-code), then click on that white arrow to run the codes in that code block.
...
Alternatively, ensure the cursor is in the relevant code block and then use the keyboard stroke `Ctrl+Enter` to execute the codes in that code block.

As mentioned in @sec-intro, the execution of these codes is not performed locally on our laptop but on the cloud using the Central Processing Unit (CPU) and the memory provided by the Google Colab.
